...
- Subarray Division (SW)
- Divisible Sum Pairs (Math) (Others)
- Drawing Book (Simulation) (Math)
- Counting Valleys (Others)
- Electronic Shop (GRE)
- Forming a Magic Square (Others)
- Picking Numbers (Array)
- Climbing the Leaderboard (BS) (TP)
- Designer PDF Viewer (Math) (Array)
- Utopian Tree (Simulation)
- Sequence Equation (Array)
- Jumping on the Clouds: Revisited (GRE) (Array)
- Empty String (Stack)
- Cut the sticks (Sorting)
- Non-Divisible Subset (GRE)
- Equalizing the Array (Array)
- Queen's Attack (Matrix) (Simulation)
- ACM ICPC Team (Bit)
- Organizing Container of Balls (Matrix) (Sorting)
...
#     'Notes: This solution came from Youtube '
...
#     'Notes: I know I can do this better'
...
#     'Notes: The code works well but not scalable, needs adjustments on efficiency / I Will try a different Approach'
...
#     '''Notes: My solution wasn't scalable enough for acquire the full score.'''
...
#         But, basically, the solution was to sum up all the balls for each type and also sum up all the balls in each container in the initial state, 
#         and this last sums will represent each container capacity.
#         The problem is solved if there are equal amount of ball per type as it is for each container capacity, 
#         by just checking that condition the solution could be returned as a 'Possible' or 'Impossible' and that's it.
#     '''
